...
export const CreateStoryBoard = ({ formData }) => {
    ...
    const storyData =
        ...
        <container className="story-page">
            ...
            <p><span className="name">{formData.name}</span> touched the largest pumpkin, and it transformed into <span className="mysterious-character">{formData.pumpkinPatchCharacter}</span>. "I can grant one wish," it said. <span className="name">{formData.name}</span> wished for <span className="wish">{formData.wishChoice}</span> and laughter for the town's cats. <span className="mysterious-character">{formData.pumpkinPatchCharacter.charAt(0).toUpperCase() + formData.pumpkinPatchCharacter.slice(1)}</span> granted <span className="name">{formData.name}</span>'s wish, and the patch came alive with happiness.</p>
